...
**Status:** draft-v0.5 (all sections drafted)
...
| 7 | Functional requirements | drafting | 37 requirements enumerated |
| 8 | Data and state | drafting | Type DB, symbol table, deps, preprocessor, docs |
| 9 | Interfaces and integrations | drafting | LSP, JSON, TOML, filesystem, Rust crates |
| 10 | UX / operator experience | drafting | Perf targets, degraded mode, commands |
| 11 | Constraints | drafting | Rust, perf, protocol, compat, testing |
| 12 | Edge cases and failure modes | drafting | Parsing, type resolution, deps, config |
| 13 | Acceptance criteria | drafting | Fixture suite, feature acceptance, info.toml |
| 14 | Assumptions / deferred / unresolved | drafting | Ongoing |
| 15 | Provenance and evidence | drafting | Ongoing |
| 16 | Risks | drafting | 6 risks identified |
...
- All game defines: `TMNEXT`, `MP4`, `MP40`, `MP41`, `TURBO`, `FOREVER`,
  `UNITED_FOREVER`, `NATIONS_FOREVER`, `MP3`
- Platform: `MANIA64`, `WINDOWS`
...
**Status:** drafting | Provenance: Decision (drafted from confirmed inputs)
...
**Status:** drafting | Provenance: Decision (drafted from confirmed inputs) + Assumption (inferred)

### 8.1 Type Database (from JSON)

Loaded at startup from OpenPlanet JSON files. Schema per type JSON:
...
This is an immutable, read-only data structure after loading. Indexed by:
- Fully qualified name (namespace + name)
- Type ID (numeric, for cross-references within the JSON)
...
**Status:** drafting | Provenance: Assumption (inferred) + Confirmed (source)
...
**Status:** drafting | Provenance: Confirmed (user) + Decision (drafted from confirmed inputs)
...
See `.7/spec-drafting/provenance-log.md` for the full provenance table.
...
**Status:** drafting | Provenance: Assumption (inferred)
