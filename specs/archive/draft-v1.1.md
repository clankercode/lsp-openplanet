...
**Status:** draft-v1.1 (verified — all review findings resolved)
...
- **D-005** Confirmed (user) + Corrected (source): Primary game target **TMNEXT**. Type info JSON files use two distinct formats (Format A for Core API, Format B for game engine types — see Section 8.1). Swapping game target = swapping the Format B file. Preprocessor `#if` handling is the key difficulty.
...
- OpenPlanet extends AngelScript with a custom preprocessor, plugin lifecycle callbacks,
  ...
  and 26 API namespaces (OpenplanetCore.json: 69 classes, 593 functions, 34 enums;
  OpenplanetNext.json: ~15,000+ game engine types in 20 namespaces).
...
during development):
- All game defines (from official docs; confirmed in plugin code: `TMNEXT`, `TURBO`, `UNITED`, `MP4`):
  `TMNEXT`, `MP4`, `MP40`, `MP41`, `TURBO`, `FOREVER`, `UNITED_FOREVER`, `NATIONS_FOREVER`, `UNITED`, `MP3`
...
- Known define sets: game (`TMNEXT`, `MP4`, `TURBO`, `UNITED` confirmed in plugins;
  `FOREVER`, `UNITED_FOREVER`, `NATIONS_FOREVER`, `MP3` etc. from official docs),
  platform (`MANIA64`, `MANIA32`, `WINDOWS`, `WINDOWS_WINE`, `LINUX`),
  build (`SERVER`, `LOGS`, `HAS_DEV`, `DEVELOPER`),
  signature (`SIG_OFFICIAL`..`SIG_DEVELOPER`),
  competition (`COMP_*` — from official docs, not observed in plugin source),
  and custom defines from `info.toml`.
...
**Format A — Core OpenPlanet API (`OpenplanetCore.json`):**
Has top-level `op` version string plus flat arrays of functions, classes, and enums.
26 namespaces: Audio, Auth, Crypto, Dev, Discord, Draw, Fids, IO, Import, Json, Math,
Meta, Net, Permissions, Reflection, Regex, SQLite, Settings, Text, Time, UI, XML,
mat3, mat4, nvg, string.
```
{
  "op": "1.25.36 (next, Public, 2023-03-29)",
  "functions": [{ "ns": "UI", "name": "Begin", "returntypedecl": "bool", "args": [...], "desc": "...", ... }],
  "classes":   [{ "id": 12345, "ns": "Net", "name": "HttpRequest", "inherits": "...", "methods": [...], "props": [...], ... }],
  "enums":     [{ "id": 43, "ns": "Net", "name": "HttpMethod", "values": {"Get": 0, "Post": 1, ...} }]
}
```

**Format B — Game engine types (`OpenplanetNext.json` etc.):**
Top-level `op` version string, `mp` (map/build date), and `ns` as a nested
namespace → type → member hierarchy. 20 namespaces: Audio, Control, Function,
Game, GameData, Graphic, Hms, Input, Meta, MetaNotPersistent, MwFoundations, Net,
Plug, Scene, Script, ShootMania, System, TrackMania, Vision, Xml. ~15,000+ types.
```
{
  "op": "1.25.36",
  "mp": "2023-03-31 22:17:46",
  "ns": {
    "MwFoundations": {
      "CMwNod": {
        "p": "",              // parent class
...
existing VS Code extension's `database.ts` (`AddTypesFromOpenplanet` for Format A,
`AddNadeoTypesFromOpenplanet` for Format B) is reference for the schema only.
...
- **Variables:** global variables (including `[Setting]`-annotated settings).
...
- C-03: OpenPlanet attribute syntax `[Setting ...]` and `[SettingsTab ...]` confirmed in real plugins. `[Persistent]` not found in any source — removed from spec. (Confirmed (source))
